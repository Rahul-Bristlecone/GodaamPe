...
: "${ABS_CONFIG_SERVICE_URL:=/config-api}"
...
if [ -f "$RUNTIME_CONFIG_TEMPLATE" ]; then
  envsubst '${USER_SERVICE_URL} ${STORE_SERVICE_URL} ${PRODUCT_SERVICE_URL} ${ABS_CONFIG_SERVICE_URL} ${COMPANY_SERVICE_URL} ${CUSTOMER_SERVICE_URL} ${ORDER_SERVICE_URL}' < "$RUNTIME_CONFIG_TEMPLATE" > "$RUNTIME_CONFIG_OUTPUT"
fi
